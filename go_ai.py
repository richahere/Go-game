...
class go_ai(object):
        ...
        for axis in direction:
            for (x_dir, y_dir) in axis:
                ...
                if x_dir != 0 and (j + x_dir < 0 or j+ x_dir >= SIZE):
                    break
                if y_dir != 0 and (i + y_dir < 0 or i + y_dir >= SIZE):
                    break
        ...
        for i in range(SIZE):
            for j in range(SIZE):
                if self.__go.getchess_map()[i][j]!=StateOfBoard.empty:
                    continue
                if not self.has_neighbour(self.__go.getchess_map()[i][j],i,j):
                    continue
                ...
                next_play.setBoard(i,j,self.__current_state)
    ...
    def evaluate(self):
        ...
        for i in range(1,SIZE-4):
            ...
            vec.append(v)
            v=[self.__go.getchess_map()[y-i][y] for y in range(i,SIZE)]
            vec.append(v)
    ...
    def evaluatePoint(self,i,j):
        ...
        if i+j==SIZE-1:
            vec.append([self.__go.getchess_map()[x][SIZE-1-x] for x in range(SIZE)])
            ...
            vec.append([self.__go.getchess_map()[x][SIZE - 1 - x+ i + j - SIZE + 1] for x in range(i + j - SIZE
                           + 1, SIZE)])
        ...
        for i in range(SIZE):
            for j in range(SIZE):
                ...
                if self.check(self.__current_state, i, j):
                    ...
                    if self.checkmate(self.__current_state) \
                        is True:
                        print ('not safe, searching other moves...')
                    elif self.checkmate(self.__current_state) \
                        is False:
                        print ('safe!!!!!!')
                        self.__go.set_board_state(i, j,self.__current_state)
                        return True

        node = go_ai(self.__go, self.__current_state,self.depth)
        score = self.alpha_beta_pruning(node)
        print (score)
        (i, j) = (node.current_i, node.current_j)
